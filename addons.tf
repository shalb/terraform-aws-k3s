resource "null_resource" "wait_cluster_ready" {
  provisioner "local-exec" {
    ...
    command = "until (aws s3 cp s3://${var.s3_bucket}/${local.cluster_dns_zone}/${local.kubeconfig_filename} /tmp/kubeconfig && for i in $(seq 10); do kubectl version --kubeconfig /tmp/kubeconfig --request-timeout=5s || exit 1; sleep 1; done) >/dev/null 2>&1; do sleep 1; echo waiting for kubeconfig; done"
  }
  depends_on = [
